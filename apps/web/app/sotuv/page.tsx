...
function SearchSelect({ items, value, onChange, placeholder, borderColor, compact, onAddNew, addLabel }: {
  ...
  const place=()=>{
    ...
    // Dropdown kamida 260px (tor katakda ham mahsulot nomi o'qilsin), ekranga sig'sin
    const width = Math.min(Math.max(r.width, 260), vw - 16);
    let left = r.left;
    if (left + width > vw - 8) left = Math.max(8, vw - 8 - width);  // o'ng chetdan oshmasin
    if(up) setPos({left,width,bottom:vh-r.top+4,listMaxH});
    else setPos({left,width,top:r.bottom+4,listMaxH});
  };
  useEffect(()=>{
    if(!open) return;
  ...
  return (
    <div ref={ref} style={{position:"relative"}}>
      <div onClick={()=>{ if(!open) place(); setOpen(o=>!o); setQ(""); }}
        style={{display:"flex",alignItems:"center",justifyContent:compact?"flex-start":"space-between",gap:compact?3:6,padding:compact?"8px 7px":"10px 14px",background:"var(--bg)",border:`1px solid ${borderColor||"var(--border)"}`,borderRadius:"var(--radius)",cursor:"pointer",fontSize:compact?13:14,fontWeight:selected?400:600,color:selected?"var(--text)":"var(--text-2)"}}>
        <span style={{overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",minWidth:0}}>{selected?selected.label:placeholder||"Tanlang..."}</span>
        <svg width={compact?11:14} height={compact?11:14} fill="none" stroke="currentColor" viewBox="0 0 24 24" style={{color:"var(--text-3)",transform:open?"rotate(180deg)":"none",transition:"transform .15s",flexShrink:0}}>
...
// Minimal sotuv narxi (tan narx) va u QAYERDAN olinganini tushuntiruvchi matn.
// Ogohlantirishda ko'rsatiladi - shunda "nega past deyapti" darrov ko'rinadi:
// tan so'mdami, yoki dollardagi tan bugungi kursga ko'paytirilganmi.
function minNarxInfo(s:SavatItem,kurs:string,mMap:Record<string,Mahsulot>):{min:number;izoh:string;kursYoq:boolean} {
  const m=mMap[s.Mahsulot_ID];
  const tanSom=num(m?.Tan_som||"0"), tanDollar=num(m?.Tan_dollar||"0"), k=num(kurs);
  const som=(v:number)=>Math.round(v).toLocaleString("ru-RU")+" so'm";
  if(s.valyuta==="som"){
    if(tanSom!==0) return {min:tanSom,izoh:som(tanSom),kursYoq:false};
    if(tanDollar>0&&k<=0) return {min:0,izoh:"kurs kiritilmagan",kursYoq:true};
    return {min:tanDollar*k,izoh:tanDollar>0?fmtUsd(tanDollar)+" x "+k.toLocaleString("ru-RU")+" = "+som(tanDollar*k):"belgilanmagan",kursYoq:false};
  }
  if(tanDollar!==0) return {min:tanDollar,izoh:fmtUsd(tanDollar),kursYoq:false};
  if(tanSom>0&&k<=0) return {min:0,izoh:"kurs kiritilmagan",kursYoq:true};
  return {min:k>0?tanSom/k:0,izoh:tanSom>0?som(tanSom)+" / "+k.toLocaleString("ru-RU")+" = "+fmtUsd(tanSom/k):"belgilanmagan",kursYoq:false};
}

function isBelowCost(s:SavatItem,kurs:string,mMap:Record<string,Mahsulot>):boolean {
  if(!s.Mahsulot_ID) return false;
  if(!mMap[s.Mahsulot_ID]) return false;
  const {min,kursYoq}=minNarxInfo(s,kurs,mMap);
  if(kursYoq) return true;               // kurssiz tan narxni hisoblab bo'lmaydi
  const narx=s.valyuta==="som"?num(s.Som_Narx):num(s.Narx);
  if(narx<=0) return min>0;
  return min>0&&narx<min;
}

// Soni 0 (yoki bo'sh) - qator ma'nosiz, bloklanadi.
// MANFIY soniga RUXSAT BERILADI: mijoz tovarni QAYTARGANDA minus bilan kiritiladi.
// Shunda: Summa manfiy -> mijoz qarzidan ayriladi; ombor hisobida -Soni -> tovar omborga qaytadi
// (lib/ombor-transfer.ts: bump(Ombor_ID, -Soni)); foyda ham teskari yozilib, asl sotuv foydasini bekor qiladi.
        ...
        {!isMobile&&somRows.length>0&&(
          <div style={{display:"grid",gridTemplateColumns:"28px 3fr 90px 130px 110px 36px",gap:8,padding:"6px 0",marginBottom:4}}>
            {["№","MAHSULOT","MIQDOR","NARX (so'm)","JAMI",""].map(h=>(
              <span key={h} style={{fontSize:isMobile?9:10,fontWeight:700,color:"var(--text-3)",letterSpacing:".03em",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{h}</span>
            ))}
          </div>
        )}
        {somRows.map((s,idx)=>{
          ...
          if(isMobile) return (
            <div key={s.id} style={{display:"grid",gridTemplateColumns:"13px minmax(0,1fr) 40px 54px minmax(46px,auto)",gap:5,alignItems:"center",marginBottom:6}}>
              ...
              {jS
                ? <span title={bc?("Tan narx: "+minNarxInfo(s,kursVal,mMap).izoh):undefined} style={{minWidth:0,fontSize:13,fontWeight:800,textAlign:"right",color:bc?"#ef4444":"#16a34a",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{bc?"past!":jS.toLocaleString("ru-RU")}</span>
          ...
          return (
            <div key={s.id} style={{display:"grid",gridTemplateColumns:"28px 3fr 90px 130px 110px 36px",gap:8,alignItems:"center",marginBottom:8}}>
              ...
              <div title={bc?("Tan narx: "+minNarxInfo(s,kursVal,mMap).izoh):(num(s.Soni)<0?"Qaytarish — summa mijoz qarzidan ayriladi, tovar omborga qaytadi":undefined)} style={{padding:"10px",background:bc?"#fef2f2":(num(s.Soni)<0?"#fff7ed":"#f0fdf4"),borderRadius:"var(--radius)",fontSize:13,fontWeight:700,textAlign:"right",color:bc?"#ef4444":(num(s.Soni)<0?"#ea580c":"#16a34a")}}>
                {bc?"Tan narxidan past!":(num(s.Soni)!==0?(jS?(num(s.Soni)<0?"↩ ":"")+jS.toLocaleString("ru-RU")+" so'm":"—"):"—")}
              </div>
        ...
        {dollarRows.map((s,idx)=>{
          ...
          return (
            <div key={s.id} style={{display:"grid",gridTemplateColumns:"28px 3fr 90px 130px 110px 36px",gap:8,alignItems:"center",marginBottom:8}}>
              ...
              <div title={bc?("Tan narx: "+minNarxInfo(s,kursVal,mMap).izoh):(num(s.Soni)<0?"Qaytarish — summa mijoz qarzidan ayriladi, tovar omborga qaytadi":undefined)} style={{padding:"10px",background:bc?"#fef2f2":(num(s.Soni)<0?"#fff7ed":"#eff6ff"),borderRadius:"var(--radius)",fontSize:13,fontWeight:700,textAlign:"right",color:bc?"#ef4444":(num(s.Soni)<0?"#ea580c":"#2563eb")}}>
                {bc?"Tan narxidan past!":(num(s.Soni)!==0?(jU?(num(s.Soni)<0?"↩ ":"")+fmtUsd(jU):"—"):"—")}
              </div>
              <button onClick={()=>onRemove(s.id)} style={{width:36,height:40,borderRadius:8,border:"none",background:"#fee2e2",color:"#ef4444",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,fontSize:20,fontWeight:700}}>−</button>
            </div>
          );
...
export default function SotuvPage() {
  ...
  // Maydonlar Klientlar sahifasidagi to'liq forma bilan bir xil
  ...
  const [newMijozAgent, setNewMijozAgent]   = useState("");
  const [newMijozBoshSom, setNewMijozBoshSom]       = useState("");
  const [newMijozBoshDollar, setNewMijozBoshDollar] = useState("");
  ...
  useEffect(()=>{
    const c=()=>setSavatMobile(window.innerWidth<768);
    c(); window.addEventListener("resize",c); return ()=>window.removeEventListener("resize",c);
  },[]);
  ...
  function openNewMijoz(query:string) {
    ...
    // Agent — sotuvdagi tanlangan agent (yo'q bo'lsa joriy foydalanuvchi) oldindan tanlanadi
    setNewMijozAgent(addAgent || user?.id || "");
    setNewMijozBoshSom("");
    setNewMijozBoshDollar("");
  ...
  async function handleAddNewMijoz() {
    ...
    const agent = newMijozAgent || addAgent || user?.id || "";
    const boshSom = newMijozBoshSom.trim();
    const boshDollar = newMijozBoshDollar.trim();
    ...
    try {
      const res=await fetch("/api/sheets",{method:"POST",headers:{"Content-Type":"application/json"},
        body:JSON.stringify({sheet:"Mijozlar",row:{
          ...
          Boshlangich_Balans_som:boshSom, Boshlangich_Balans_dollar:boshDollar,
        }})});
      if(!res.ok) throw new Error("Mijoz saqlanmadi");
      // Lokal ro'yxatga darhol qo'shiladi (reload kutmasdan)
      setMijozlar(prev=>[...prev,{Mijoz_ID:id,Ism:ism,Telefon:newMijozTel.trim(),Agent:agent,Boshlangich_Balans_som:boshSom,Boshlangich_Balans_dollar:boshDollar}]);
      afterWrite("Mijozlar");
      // Boshqa agent tanlangan bo'lsa — sotuv agentini ham moslab, mijoz dropdownda ko'rinishini ta'minlaymiz
      if(agent && agent!==addAgent) setAddAgent(agent);
      setAddMijoz(id);
              ...
              {isMobile?(
                ...
                <div style={{position:"sticky",top:56,zIndex:10,background:"var(--white)",borderRadius:"var(--radius-xl) var(--radius-xl) 0 0"}}>
                  <div style={{display:"flex",alignItems:"center",gap:10,padding:"14px 20px",borderBottom:"1px solid var(--border)",flexWrap:"wrap"}}>
                    ...
                    <select value={filterYil} onChange={e=>setFilterYil(e.target.value)} style={{width:"auto",padding:"8px 12px",border:"1px solid var(--border)",borderRadius:"var(--radius)",fontSize:13,fontWeight:600,background:"var(--white)",cursor:"pointer",outline:"none"}}>
                      <option value="">Barcha yillar</option>
                      {years.map(y=><option key={y} value={y}>{y}</option>)}
                    </select>
                    <select value={filterOy} onChange={e=>setFilterOy(e.target.value)} style={{width:"auto",padding:"8px 12px",border:"1px solid var(--border)",borderRadius:"var(--radius)",fontSize:13,fontWeight:600,background:"var(--white)",cursor:"pointer",outline:"none"}}>
                      <option value="">Barcha oylar</option>
                      {OY_NOMLARI.map((n,i)=><option key={i+1} value={String(i+1)}>{n}</option>)}
                    </select>
      ...
      {addOpen&&(
        <div style={addOverlay}>
          <div style={addModalBox}>
            ...
            {(() => { const bad=savat.filter(s=>isBelowCost(s,addKurs,mMap)).map(s=>{const n=mMap[s.Mahsulot_ID]?.Nomi||"";return n?n+" (tan: "+minNarxInfo(s,addKurs,mMap).izoh+")":"";}).filter(Boolean); return bad.length?(<div style={{padding:"9px 20px",background:"#fef2f2",borderTop:"1px solid #fecaca",fontSize:12.5,fontWeight:700,color:"#dc2626"}}>⚠️ Tan narxidan past — saqlab bo&apos;lmaydi: {bad.join(", ")}. Narxni to&apos;g&apos;rilang.</div>):null; })()}
            {(() => { const bad=savat.filter(isBadSoni).map(s=>mMap[s.Mahsulot_ID]?.Nomi||"").filter(Boolean); return bad.length?(<div style={{padding:"9px 20px",background:"#fef2f2",borderTop:"1px solid #fecaca",fontSize:12.5,fontWeight:700,color:"#dc2626"}}>⚠️ Soni 0 — saqlab bo&apos;lmaydi: {bad.join(", ")}. Sonini kiriting (qaytarish uchun minus bilan).</div>):null; })()}
      ...
      {newMijozOpen&&(
        <div style={{position:"fixed",inset:0,zIndex:1100,background:"rgba(15,42,76,.42)",backdropFilter:"blur(4px)",display:"flex",alignItems:isMobile?"flex-end":"center",justifyContent:"center",padding:isMobile?0:20}}
          ...
          <div onClick={e=>e.stopPropagation()}
            style={{background:"var(--white)",width:"100%",maxWidth:isMobile?"100%":440,borderRadius:isMobile?"20px 20px 0 0":16,display:"flex",flexDirection:"column",maxHeight:isMobile?"92dvh":"88vh"}}>
            {isMobile&&<div style={{width:40,height:4,borderRadius:2,background:"var(--border)",margin:"12px auto 0"}}/>}
            <div style={{display:"flex",alignItems:"center",gap:12,padding:"16px 20px",borderBottom:"1px solid var(--border)"}}>
              <button onClick={()=>setNewMijozOpen(false)} style={{width:36,height:36,borderRadius:8,border:"1px solid var(--border)",background:"var(--white)",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,color:"var(--text-2)"}}>
                <svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12"/></svg>
              </button>
              <h2 style={{fontSize:16,fontWeight:800}}>Yangi mijoz</h2>
            </div>
            <div style={{padding:"18px 20px",display:"flex",flexDirection:"column",gap:18,overflowY:"auto"}}>
              {/* Asosiy ma'lumot */}
              <div style={{display:"flex",flexDirection:"column",gap:12}}>
                <p style={{fontSize:11,fontWeight:700,color:"var(--text-3)",letterSpacing:".05em",margin:0}}>ASOSIY MA&apos;LUMOT</p>
                <div>
                  ...
                  <input value={newMijozIsm} onChange={e=>setNewMijozIsm(e.target.value)} placeholder="Mijoz nomi" autoFocus
                    ...
                    style={{width:"100%",padding:"10px 14px",border:"1px solid var(--border)",borderRadius:"var(--radius)",fontSize:14,outline:"none",boxSizing:"border-box",background:"var(--bg)"}}/>
                </div>
                <div>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:8}}>
                <p style={{fontSize:11,fontWeight:700,color:"var(--text-3)",letterSpacing:".05em",margin:0}}>VALYUTA</p>
                <div style={{display:"flex",gap:8}}>
              ...
              {/* Agent */}
              <div style={{display:"flex",flexDirection:"column",gap:8}}>
                <p style={{fontSize:11,fontWeight:700,color:"var(--text-3)",letterSpacing:".05em",margin:0}}>AGENT</p>
                <select value={newMijozAgent} onChange={e=>setNewMijozAgent(e.target.value)}
              ...
              {/* Boshlang'ich qoldiq */}
              <div style={{display:"flex",flexDirection:"column",gap:12}}>
                <p style={{fontSize:11,fontWeight:700,color:"var(--text-3)",letterSpacing:".05em",margin:0}}>BOSHLANG&apos;ICH QOLDIQ</p>
                <div>
                  <label style={{fontSize:12,fontWeight:600,color:"var(--text-2)",display:"block",marginBottom:6}}>So&apos;m</label>
                  <input value={newMijozBoshSom} onChange={e=>setNewMijozBoshSom(e.target.value)} placeholder="0" inputMode="decimal"
                    style={{width:"100%",padding:"10px 14px",border:"1px solid var(--border)",borderRadius:"var(--radius)",fontSize:14,outline:"none",boxSizing:"border-box",background:"var(--bg)"}}/>
                </div>
                <div>
                  <label style={{fontSize:12,fontWeight:600,color:"var(--text-2)",display:"block",marginBottom:6}}>Dollar ($)</label>
                  <input value={newMijozBoshDollar} onChange={e=>setNewMijozBoshDollar(e.target.value)} placeholder="0" inputMode="decimal"
                    style={{width:"100%",padding:"10px 14px",border:"1px solid var(--border)",borderRadius:"var(--radius)",fontSize:14,outline:"none",boxSizing:"border-box",background:"var(--bg)"}}/>
                </div>
              </div>
            </div>
            <div style={{display:"flex",gap:10,padding:"16px 20px",borderTop:"1px solid var(--border)",paddingBottom:isMobile?"max(16px, env(safe-area-inset-bottom))":16}}>
              <button className="btn btn--outline" style={{flex:1}} onClick={()=>setNewMijozOpen(false)}>Bekor qilish</button>
      ...
      {detailSotuv&&(
        <div style={modalOverlay} onClick={()=>setDetailSotuv(null)}>
          <div style={modalBox} onClick={e=>e.stopPropagation()}>
            ...
            {(() => { const bad=editSavat.filter(s=>isBelowCost(s,editKurs,mMap)).map(s=>{const n=mMap[s.Mahsulot_ID]?.Nomi||"";return n?n+" (tan: "+minNarxInfo(s,editKurs,mMap).izoh+")":"";}).filter(Boolean); return bad.length?(<div style={{padding:"9px 20px",background:"#fef2f2",borderTop:"1px solid #fecaca",fontSize:12.5,fontWeight:700,color:"#dc2626"}}>⚠️ Tan narxidan past — saqlab bo&apos;lmaydi: {bad.join(", ")}. Narxni to&apos;g&apos;rilang.</div>):null; })()}
            {(() => { const bad=editSavat.filter(isBadSoni).map(s=>mMap[s.Mahsulot_ID]?.Nomi||"").filter(Boolean); return bad.length?(<div style={{padding:"9px 20px",background:"#fef2f2",borderTop:"1px solid #fecaca",fontSize:12.5,fontWeight:700,color:"#dc2626"}}>⚠️ Soni 0 — saqlab bo&apos;lmaydi: {bad.join(", ")}. Sonini kiriting (qaytarish uchun minus bilan).</div>):null; })()}
            <div style={{display:"flex",gap:10,padding:"16px 20px",borderTop:"1px solid var(--border)",paddingBottom:isMobile?"max(16px, env(safe-area-inset-bottom))":16}}>
