...
// 80mm termal chek (Xprinter M817). Ko'rinishi A4 chek bilan bir xil,
// lekin @page 80mm — telefon/Bluetooth termal printerga chop etiladi.
  ...
  async function handleShare() {
    ...
    try {
      ...
      if (pngBlob) {
        ...
        } catch { /* clipboard bo'lmasa — PDF zaxira */ }
      }
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        @media print {
          ...
          *, th, td, span, div, p, strong { font-weight: 700 !important; }
          /* 80mm roll: chek to'liq qog'oz eniga (76mm) chiqadi */
          .chek-sheet { width: 80mm; }
          .chek-wrap { width: 76mm; max-width: 76mm; min-height: auto; margin: 0 auto; }
          ...
          .chek-table th:nth-child(1), .chek-table td:nth-child(1) { width: 7% !important; }
          .chek-table th:nth-child(2), .chek-table td:nth-child(2) { width: 40% !important; text-align: left !important; }
          .chek-table th:nth-child(3), .chek-table td:nth-child(3) { width: 11% !important; }
          .chek-table th:nth-child(4), .chek-table td:nth-child(4) { width: 19% !important; }
          .chek-table th:nth-child(5), .chek-table td:nth-child(5) { width: 23% !important; }
          .chek-table th:nth-child(6), .chek-table td:nth-child(6) { display: none !important; }
          .chek-header { justify-content: center; padding: 6px 6px; background-color: #1a2744 !important; }
          .chek-header__left { align-items: center; text-align: center; }
          .chek-header__sub { display: none; }
          .chek-header__logo, .chek-header__logo span { font-size: 17px; color: #fff !important; }
          .chek-info { padding: 5px 6px; gap: 2px 8px; }
          .chek-info__item { gap: 0; }
          .chek-info__label { font-size: 9px; }
          .chek-info__value { font-size: 10px; }
          .chek-section-title { padding: 3px 6px 2px; font-size: 9px; letter-spacing: 1px; }
          .chek-table-wrap { padding: 0 4px 3px; }
          .chek-table { border: 1.2px solid #000 !important; }
          .chek-table thead th { padding: 3px 2px; font-size: 8px; letter-spacing: 0; color: #fff !important; background-color: #1a2744 !important; border: 0.8px solid #2d3a5c !important; }
          .chek-table tbody td { padding: 3px 2px; font-size: 8.5px; border: 0.8px solid #555 !important; }
          .chek-table tfoot td { padding: 3px 2px; font-size: 9px; border: 0.8px solid #000 !important; }
          .chek-balance { margin: 4px 4px 6px; border-radius: 0; }
          .chek-balance__header { padding: 4px 6px; font-size: 8.5px; letter-spacing: 1px; color: #fff !important; background-color: #1a2744 !important; }
          .chek-balance__header-cell { color: #fff !important; font-weight: 900 !important; }
          .chek-balance__row { background-color: #fff !important; }
          .chek-balance__cell { padding: 3px 6px; font-size: 9px; color: #000 !important; background-color: #fff !important; }
          .chek-balance__cell-label { font-size: 9px; color: #000 !important; font-weight: 800 !important; }
          .chek-balance__cell-val { font-size: 9px; color: #000 !important; font-weight: 800 !important; }
          .chek-balance__row--total .chek-balance__cell { font-size: 10px; background-color: #1a2744 !important; }
          ...
          .chek-footer { padding: 5px 0; font-size: 8px; margin: 0 6px; }
        }
      `}</style>
      ...
      <>
        <div className="chek-header">
          <div className="chek-header__left">
            ...
            <div className="chek-header__sub">Hisob · Faktura · 80mm</div>
          </div>
        ...
        <div className="chek-info">
          <div className="chek-info__item"><span className="chek-info__label">Sana</span><span className="chek-info__value">{sana||"—"}</span></div>
          <div className="chek-info__item"><span className="chek-info__label">Agent</span><span className="chek-info__value">{agentNomi||"—"}</span></div>
          <div className="chek-info__item"><span className="chek-info__label">Mijoz</span><span className="chek-info__value">{mijozIsm||"—"}</span></div>
          <div className="chek-info__item"><span className="chek-info__label">Telefon</span><span className="chek-info__value">{mijozTel||"—"}</span></div>
        </div>
              ...
              <tbody>
                {!rowsReady ? (
                  ...
                  <tr key={r.Savat_ID||i}>
                    ...
                    <td>{fmtSom(num(r.Som_Narx))}</td>
                    <td className="amount">{fmtSom(num(r.Summa_som))}</td>
                    <td className="left" style={{color:"#7a80a0"}}>{r.Izoh||""}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr><td colSpan={4} className="label">Jami:</td><td>{fmtSom(thisSom)}</td><td></td></tr>
              </tfoot>
            </table>
          </div>
        )}
              ...
              <tbody>
                {!rowsReady ? (
                  ...
                  <tr key={r.Savat_ID||i}>
                    ...
                    <td className="name">{mMap[r.Mahsulot_ID]?.Nomi||r.Mahsulot_ID}</td>
                    <td>{num(r.Soni).toFixed(1)}</td>
                    <td>{fmtUsd(num(r.Narx))}</td>
                    <td className="amount">{fmtUsd(num(r.Summa))}</td>
                    <td className="left" style={{color:"#7a80a0"}}>{r.Izoh||""}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr><td colSpan={4} className="label">Jami:</td><td>{fmtUsd(thisDollar)}</td><td></td></tr>
              </tfoot>
            </table>
          </div>
        )}
        ...
        <div className="chek-balance">
          ...
          <div className="chek-balance__row">
            {showSomBal    && <div className="chek-balance__cell"><span className="chek-balance__cell-label">To&apos;lov</span><span className="chek-balance__cell-val" style={{color:"#16a34a"}}>− {fmtSom(tolovSom)}</span></div>}
            {showDollarBal && <div className="chek-balance__cell"><span className="chek-balance__cell-label">To&apos;lov</span><span className="chek-balance__cell-val" style={{color:"#16a34a"}}>− {fmtUsd2(tolovDollar)}</span></div>}
          </div>
          )}
          <div className="chek-balance__row chek-balance__row--total">
            {showSomBal    && <div className="chek-balance__cell"><span className="chek-balance__cell-label">Yakuniy balans</span><span className="chek-balance__cell-val">{fmtSom(totalSom+thisSom-tolovSom)}</span></div>}
            {showDollarBal && <div className="chek-balance__cell"><span className="chek-balance__cell-label">Yakuniy balans</span><span className="chek-balance__cell-val">{fmtUsd2(totalDollar+thisDollar-tolovDollar)}</span></div>}
          </div>
        </div>
